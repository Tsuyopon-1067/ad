...
public class Rp01_B1 {
    // static MySearch mySearch;
    static BinarySearch mySearch;
     ...
     * ファイル読み込み
     * boolean commandExecute(String line)
     * コマンド読み込み
    ...
    private static boolean commandExecute(String line) {
        StringTokenizer token = new StringTokenizer(line, " "); // スペース区切りで分割
        int tokenCount = token.countTokens(); // スペース区切りの要素数
        if (tokenCount > 0) {
            String command = new String(token.nextToken()); // 一番左はcommand
            if ((command.equals("insert") || command.equals("ins"))
                ...
                while (token.hasMoreTokens()) {
                    newData.append(token.nextToken());
                    newData.append(" ");
                }
            ...
            } else if ((command.equals("search") || command.equals("find")) // 追加部分 範囲検索
                    && tokenCount == 3) {
                /* search, find */
                String strL = token.nextToken();
                String strR = token.nextToken();
                MyData[] results = mySearch.searchRange(strL, strR);
                if (results != null) {
                    for (MyData result : results) {
                        System.out.println("  key (" + result.getKey() + ") is found.");
                        System.out.println("  " + result);
                    }                    
                } else {
            ...
            /* for searchIndex() method test */
                ...
                System.out.println(
                        "  searchIndex(" + searchKey + "): " + result + " (expected output: " + correctResult + ")");
