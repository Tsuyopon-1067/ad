class Rp4_1 {
    ...
    static public void enqueueTest(MyQueue2 mq, String data, boolean estimated) {
        enqueueCount++;
        if (test("   enqueue(" + data + ")", mq.enqueue(data), estimated))
            enqueueCorrectCount++;
    }

    static public void dequeueTest(MyQueue2 mq, String estimated) {
        dequeueCount++;
        if (test("   dequeue()", mq.dequeue().getName(), estimated))
            dequeueCorrectCount++;
    }

    static public void isEmptyTest(MyQueue2 mq, boolean estimated) { //
        isEmptyCount++;
