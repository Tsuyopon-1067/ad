class Ex4_14 {
    ...
    static public void enqueueTest(MyQueue1 mq, String data, boolean estimated) {
        enqueueCount++;
        if (test("   enqueue(" + data + ")", mq.enqueue(data), estimated))
            enqueueCorrectCount++;
    }

    static public void dequeueTest(MyQueue1 mq, String estimated) {
        dequeueCount++;
        if (test("   dequeue()", mq.dequeue(), estimated))
            dequeueCorrectCount++;
    }

    static public void isEmptyTest(MyQueue1 mq, boolean estimated) { //
        isEmptyCount++;
    ...
    public static void main(String[] args) {
        // オリジナル MyQueue1.java にあった，テスト用のメインルーチン
        testCase1();
        printAllTestResult();
    }
}